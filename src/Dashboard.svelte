<script>
  ...
  $: selectedGraph = 0;
  $: selectedCountries = selectedDataset.options.defaultSelected
    ? selectedDataset.options.defaultSelected
    : [];
  $: highlightedCountry = "";
  $: options = {
    scale: "logarithmic",
  ...
  $: entityList = utils.serialize(
    selectedDataset.timeSeries[0].url,
    selectedDataset.options
  );
  $: currentData = utils.serialize(
    selectedDataset.timeSeries[selectedGraph].url,
    selectedDataset.options
  );

  function toggleCountry(e) {
    const index = selectedCountries.findIndex(
      label => label === e.detail.country.name
    );
    highlightedCountry =
      highlightedCountry === e.detail.country.name ? "" : highlightedCountry;
    selectedCountries =
      index >= 0
        ? selectedCountries.filter(c => c !== e.detail.country.name)
        : [...selectedCountries, e.detail.country.name];
  }
  function toggleHighlight(e) {
    highlightedCountry =
      highlightedCountry === e.detail.country.name ? "" : e.detail.country.name;
  }
  ...
  function setDataset(ind) {
    selectedDatasetIndex = ind;
  }
</script>
    ...
    <div>
      ...
      <ul class="dataset-list">
        {#each config.datasets as dataset, i}
          <li
            class="dataset-opt"
            class:active-dataset={i == selectedDatasetIndex}
            on:click={e => setDataset(i)}>
            {dataset.title}
          </li>
        {/each}
      </ul>
      ...
      <section class="graph">
        <ul class="graph-selector">
          {#each selectedDataset.timeSeries as graph, i}
            <li>
              <button
                on:click={() => changeGraph(i)}
                class={i === selectedGraph ? 'selected' : ''}>
                {graph.label}
              </button>
            </li>
          {/each}
          ...
          <EvolutionGraph
            countries={utils
              .serializeSelected(
                ...
                selectedCountries,
                highlightedCountry,
                selectedDataset.timeSeries[selectedGraph].dayZero
              )
